(ns structuralsteel.operation
  "OperationActor -- one structural-steel-fabrication operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Structural Fabrication Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the
  Structural Fabrication Governor (:govern) and the rollout phase
  ...
  One graph run = one structural-steel-fabrication operation (intake ->
  ...
  decision to a human operator (the certified welding inspector). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-assembly`/`:actuation/issue-
  fabrication-certificate` ALWAYS reach this node when the governor is
  clean -- see `structuralsteel.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [structuralsteel.structuralsteeladvisor :as structuralsteeladvisor]
            [structuralsteel.governor :as governor]
...
(defn build
  ...
  `structuralsteel.store/Store`).
  opts:
    :advisor      -- a `structuralsteel.structuralsteeladvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (structuralsteeladvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Structural Fabrication Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (structuralsteeladvisor/-advise advisor store request)]
            {:proposal p :audit [(structuralsteeladvisor/trace request p)]})))

      ;; Structural Fabrication Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
